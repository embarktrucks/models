...
def extract_cuboid_bbox(cuboid_data, image_shape):
    ...
    ymax = None
    xmax = None
    is_shoulder = cuboid_data.get('on_shoulder', False)
    for v in cuboid_data['vertices']:
        if v['description'] == 'face-topleft':
            xmin = int(v['x'])
            ymin = int(v['y'])
    ...
    return label, label_text, is_shoulder, xmin, ymin, xmax, ymax
...
def extract_cuboid_bboxes(cuboids,
    ...
    for cuboid_data in cuboids:
        label, label_text, is_shoulder, xmin, ymin, xmax, ymax = extract_cuboid_bbox(cuboid_data, image_shape)
        if label is None:
            continue
