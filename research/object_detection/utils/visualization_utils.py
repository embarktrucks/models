...
def draw_bounding_boxes_on_image_tensors(name,
                                         images_list,
    ...
    if groundtruth_scores_list is None:
        groundtruth_scores_list = [None] * len(images_list)
    ...
    for image, bboxes, classes, scores in zip(images_list, groundtruth_boxes_list, groundtruth_classes_list, groundtruth_scores_list):
        if scores is None:
            scores = tf.ones_like(classes,
                                  dtype=tf.float32,
                                  name='ground_truth_scores')
        overlay = tf.cast(image, tf.uint8)
        overlay = draw_bounding_boxes_on_image_tensor(overlay,
                                                      bboxes,
                                                      classes,
                                                      scores,
                                                      category_index,
        ...
        overlay = tf.expand_dims(tf.cast(overlay, image.dtype), 0)
        tf.summary.image(name + "/%i" % i,
                         overlay,
                         1)
        i += 1


def draw_bounding_boxes_on_image_tensor(image,
                                        boxes,
                                        classes,
                                        scores,
                                        category_index,
    ...
    Args:
      ...
      boxes: [max_detections, 4] float32 tensor of detection boxes.
      classes: [max_detections] int tensor of detection classes. Note that
        classes are 1-indexed.
      scores: [max_detections] float32 tensor of detection scores.
