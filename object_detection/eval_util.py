...
def visualize_aggregated_results(result_lists,                                 
                                 global_step,
                                 summary_dir,                                 
                                 categories,
                                 label_id_offset=0,
    ...
    precisions_per_class, recalls_per_class, sorted_scores_per_class = (evaluator.evaluate())
    summary_writer = tf.summary.FileWriter(summary_dir, filename_suffix='.pr_curve')
    image_summaries = []
    for klass_idx, (precision, recall, scores) in enumerate(zip(precisions_per_class, recalls_per_class, sorted_scores_per_class)):
      pr_curve = vis_utils.plot_precision_recall(precision, recall, scores)
      tag = 'pr_curve_' + str(klass_idx)
      print 'image: ', tag
      summary = tf.Summary(value=[
...
def evaluate_detection_results_pascal_voc(result_lists,
    ...
    for idx, image_id in enumerate(image_ids):
        ...
        evaluator.add_single_detected_image_info(
            ...
            result_lists['detection_scores'][idx],
            result_lists['detection_classes'][idx] - label_id_offset)
    per_class_ap, mean_ap, _, _, per_class_corloc, mean_corloc, _, _, _ = (
        evaluator.evaluate())
...
def visualize_detection_results(result_dict,
    ...
    summary_writer = tf.summary.FileWriter(summary_dir, filename_suffix='.bbox_image')
    summary_writer.add_summary(summary, global_step)
    summary_writer.close()
...
def run_checkpoint_once(tensor_dict,
                        ...
                        aggregated_result_visualizer=None):
    ...
    with tf.contrib.slim.queues.QueueRunners(sess):
        try:
            ...
            aggregated_result_visualizer(result_lists, global_step, summary_dir)
    ...
    while True:
        ...
        if not model_path:
            ...
            run_checkpoint_once(tensor_dict, update_op, summary_dir,
                                ...
                                keys_to_exclude_from_results,
                                aggregated_result_visualizer=aggregated_result_visualizer)
